...
def f1_score(ground_truth_list, prediction_list):
    """
    Function to calculate class and Macro F1 score.

    :param ground_truth_list: a list containing ground truth values of images
    :param prediction_list: a list containing model predictions.
    :return: a list of float and a float value as tuple representing Classes and macro F1 scores
    """
...
parser.add_argument("-tp", "--train_path",
                    help="Path of your training set, can be relative or full, default points to continuum dataset",
                    type=str, default='dataset/continuum/train')
parser.add_argument("-ts", "--test_path",
                    help="Path of your testing set, can be relative or full, default points to continuum dataset",
                    type=str, default='dataset/continuum/test')
...
def main(args):
    ...
    if args.mode == 'TRAIN':
        ...
        train_loss_list, train_accuracy_list, valid_loss_list, valid_accuracy_list = network.train_network(
            train_data,
            validation_data,
            args.epochs)
        with open('test.txt', 'w+') as test_log:
            test_log.write(
                'Train_lostt_list: {}\nTrain_accuracy_list: {}\nValidation_loss_list: {}\nvalidation_accuracy_list: {}'
                    .format(train_loss_list, train_accuracy_list, valid_loss_list, valid_accuracy_list))
        ...
        with open('test.txt', 'w+') as test_log:
            test_log.write(
                'Accuracy: {}\nLoss: {}\nClass F1 score: {}\nMacro F1 score: {}'.format(accuracy, loss, class_score,
                                                                                        macro_score))
